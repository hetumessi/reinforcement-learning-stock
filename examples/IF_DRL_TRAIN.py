...
def init(context):
    ...
    context.s1 = 'IF88'
    context.train_data_obs = [0]
    context.train_data_rewards = []
    context.end = datetime.datetime(2016, 12, 29)
    context.last_price = 1
    # context.model = A2C.load("../IF8888model_A2C_20210603")
...
def handle_bar(context, bar_dict):
    ...
    price_data = history_bars(context.s1, 21, '1d',
                              ...
                              include_now=True)
    # 此时价格
    now_price = price_data[len(price_data)-1][4]
    # 和上一个时间步价格的变化率
    change_rate = (now_price - context.last_price) / context.last_price
    # 更新上次价格为此时价格
    context.last_price = now_price
    ...
    context.train_data_obs.append(obs)
    context.train_data_rewards.append(change_rate)
    if context.now >= context.end:
        pickle.dump(context.train_data_obs, open("./train_obs.pkl", 'wb'))
        pickle.dump(context.train_data_rewards, open("./train_reward.pkl", 'wb'))
